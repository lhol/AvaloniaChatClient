...
/// <summary>Aggregierte Metadaten der letzten Assistenten-Antwort in einer Session.</summary>
public partial class SessionMetadata : ObservableObject
{
    ...
    public bool HasData => ServerName is not null || ModelName is not null || TtftMs is not null;

    public void UpdateFrom(ChatMessageViewModel msg)
    {
    ...
    public partial class ChatSessionViewModel : ViewModelBase
    {
        ...
        [ObservableProperty] private bool _showMetadata = true;          // Panel sichtbar by default
        [ObservableProperty] private bool _multilineInput = false;        // G-05
        [ObservableProperty] private SessionMetadata _currentMetadata = new();
        ...
        public async Task LoadHistoryAsync()
        {
            try
            {
                ...
                // Metadaten aus letzter Assistenten-Nachricht befüllen
                var lastAssistant = Messages.LastOrDefault(m => m.IsAssistant);
                if (lastAssistant is not null)
                    CurrentMetadata.UpdateFrom(lastAssistant);
            }
            catch (Exception ex)
            {
                Avaiu.Services.AppErrorService.Instance.Report("ChatSessionViewModel.LoadHistory", ex);
            }
        }
        ...
        private async Task SendAsync()
        {
                ...
                await foreach (var chunk in _api.SendMessageAsync(SessionId, text, _streamCts.Token))
                {
                    if (chunk.IsDone)
                    {
                        ...
                        else
                        {
                            ...
                            CurrentMetadata.UpdateFrom(assistantMsg);
                        }
                        break;
                    }
                    if (chunk.Delta is not null)
                    {
                        ...
                        if (chunk.ServerName is not null) assistantMsg.ServerName = chunk.ServerName;
                        if (chunk.ModelName is not null) assistantMsg.ModelName = chunk.ModelName;
                        ScrollToBottomRequested?.Invoke();
                    }
                }
            }
            ...
            {
                Avaiu.Services.AppErrorService.Instance.Report("ChatSessionViewModel.Send", ex);
